...
import { TAsyncQueueEvent } from "./type/TAsyncQueueTaskEventHandlers";
import createId from "./CreateId";

/*
    Bazlama Web Async Queue
    Async Queue with timeout, loading mask and event handling
    2024-08-12
    Version 1.0
    muratbudun@gmail.com

    usage:
        const queue = new BazlamaAsyncQueue(5000)
        queue.enqueue(async (controller: AbortController, payload?: any) => { })
        queue.enqueue({id: "task-1", name: "Task 1", task: async (controller: AbortController, payload?: any) => { }})
        queue.onError((error: any, task: IAsyncQueueTask) => { })
        queue.onTimeout((task: IAsyncQueueTask) => { })

    flow:
       [enqueue] -> [start] -> <loading-mask> -> | [success] | -> [complete]
                                                 | [fail]    |
                                                 | [timeout] |
                                                 | [cancel]  |
*/
